...
function renderRecentRegistrations() {
  ...
  if (!items.length) {
    container.innerHTML =
      '<p class="empty-state">Nenhum cadastro foi salvo neste navegador.</p>'
    return
  }
...
export function setupVolunteerForm() {
  ...
  if (form) {
    form.addEventListener("submit", (event) => {
      ...
      status.innerHTML =
        '<p class="success-box">Cadastro salvo com sucesso neste navegador.</p>'
  ...
  if (clearButton) {
    clearButton.addEventListener("click", () => {
      ...
      if (status) {
        status.innerHTML =
          '<p class="success-box">Cadastros locais removidos.</p>'
      }
    })
  }
}
